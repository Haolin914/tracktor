...
def batched_nms_npu(boxes, scores, idxs, iou_threshold):
    ...
    print("yixiashihunhe")
    print(torch.cat([boxes, scores[..., None]], 1).shape, torch.cat([boxes, scores[..., None]], 1).dtype, torch.cat([boxes, scores[..., None]], 1).storage().npu_format())
    print(iou_threshold)
    ...
    print("the shape of result", keep_mask.shape)
    return keep_mask
...
def batched_nms(
    ...
    print("running the nm222s now!")
    result_mask = scores.new_zeros(scores.size(), dtype=torch.bool)
    for id in torch.jit.annotate(List[int], torch.unique(idxs).cpu().tolist()):
        mask = (idxs == id).nonzero().view(-1)
        keep = nms(boxes[mask], scores[mask], iou_threshold)
        result_mask[mask[keep]] = True
    print("running the nms3333 now!")

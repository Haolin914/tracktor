...
class ReIDNetwork(ResNet):
    ...
    def sum_losses(self, batch):
        ...
        if self.loss == "cross_entropy":
            ...
            tar_pos = Variable(torch.ones(out_pos.size(0)).view(-1,1).npu())
            tar_neg = Variable(torch.zeros(out_pos.size(0)).view(-1,1).npu())
            ...
            pos_weights = Variable(torch.zeros(dist.size()).npu())
            neg_weights = Variable(torch.zeros(dist.size()).npu())
            for i in range(dist.size(0)):
                # make by line
                mask = torch.zeros(dist.size()).byte().npu()
                mask[i] = 1
        ...
        if self.prec_at_k:
            ...
            losses['prec_at_k'] = Variable(k_loss.npu())

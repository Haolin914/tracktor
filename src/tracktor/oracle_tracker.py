...
class OracleTracker(Tracker):
	...
	def add(self, new_det_pos, new_det_scores, new_det_features, blob):
		...
		for t in self.tracks[-num_new:]:
			gt = blob['gt']
			boxes = torch.cat(list(gt.values()), 0).npu()
	...
	def regress_tracks(self, blob):
		...
		return torch.Tensor(s[::-1]).npu()

	def reid(self, blob, new_det_pos, new_det_scores):
		new_det_features = [torch.zeros(0).npu() for _ in range(len(new_det_pos))]
		...
		if self.do_reid:
			...
			if len(self.inactive_tracks) >= 1:
				...
				keep = torch.Tensor([i for i in range(new_det_pos.size(0)) if i not in assigned]).long().npu()
				if keep.nelement() > 0:
					new_det_pos = new_det_pos[keep]
					new_det_scores = new_det_scores[keep]
					new_det_features = new_det_features[keep]
				else:
					new_det_pos = torch.zeros(0).npu()
					new_det_scores = torch.zeros(0).npu()
					new_det_features = torch.zeros(0).npu()

			if len(self.inactive_tracks) >= 1 and self.reid_oracle:
				gt = blob['gt']
				gt_pos = torch.cat(list(gt.values()), 0).npu()
				gt_ids = list(gt.keys())
	...
	def nms_oracle(self, blob, person_scores):
		...
		if len(self.tracks):
			...
			keep = torch.Tensor([i for i in range(person_scores.size(0)) if i not in index_remove]).long().npu()
	...
	def step(self, blob):
		...
		if self.public_detections:
			...
			if dets.nelement() > 0:
				...
				boxes = scores = torch.zeros(0).npu()
		else:
		...
		if boxes.nelement() > 0:
			...
			if self.kill_oracle:
				gt = blob['gt']
				gt_boxes = torch.cat(list(gt.values()), 0).npu()
				...
				inds = torch.LongTensor(matched).npu()
			else:
				# Filter out tracks that have too low person score
				inds = torch.gt(scores, self.detection_person_thresh).nonzero().view(-1)
		else:
			inds = torch.zeros(0).npu()
		...
		if inds.nelement() > 0:
			...
			det_pos = torch.zeros(0).npu()
			det_scores = torch.zeros(0).npu()
		...
		if len(self.tracks):
			...
			if len(self.tracks):
				...
				if self.kill_oracle:
					...
					keep = torch.arange(nms_inp_reg.size(0)).long().npu()
				else:

...
def save_checkpoint(
    model, state, save_dir, is_best=False, remove_module_from_keys=False
):
    r"""Saves checkpoint.
    ...
    torch.save(model, 'log/model/whole_model.pth')
...
def load_checkpoint(fpath):
    ...
    #map_location = None if torch.npu.is_available() else 'cpu'
    map_location = 'npu:2' if torch.npu.is_available() else 'cpu'
    try:
